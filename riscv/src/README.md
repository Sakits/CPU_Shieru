...
5. IF 能别嵌套就别嵌套
6. 尝试不用 `@always(*)`，只用 `generate`
7. 尽量不用 `reg = `，能用 `assign wire = ` 的就用
8. 由于是电路，所以一个 `wire` 或 `reg` 都只能被赋值一次，并且只能出现在一个块中
9. 在 `@always(*)` 中只用 `reg = `，在 `@always(posedge clk)` 中只用 `reg <=`
10. 需要自己给自己赋值的，必须使用 `<=`
11. 两种解决方法，防止 `if-else` 综合出 `latch`
   1. 组合逻辑中的 `if-else` 结构必须完整，每个变量赋值都必须同时出现在 `if` 和 `else` 中，消耗逻辑门较多
   2. 需要出现在 `if-else` 中的变量一定要赋初值，然后用有优先级的 `if-else` 判断，时延较长
12. 只有两个分支的可以用三目运算符，多分支的最好还是用 `if` 或者 `case`
13. 善用 `generate` 生成一层组合逻辑
14. 逻辑门消耗：加法器 > 比较器 > 选择器
    因此先选择，再比较，再作加法
